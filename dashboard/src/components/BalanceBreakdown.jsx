    ...
    return {
      ...
      parts: [
        { label: "Boshlang'ich qoldiq", value: Number(d.cashOpening?.amount || 0),
          ...
          opening: 'cash' },
        // Bu yozuvlarning HAMMASI naqd qoldiq sahifasida (gen_info?tab=cash)
        // "Butun tarix" rejimida ko'rinadi. Kassir bo'limi faqat bugungisini
        // ko'rsatgani uchun eski pulni u yerdan topib bo'lmasdi.
        { label: 'Kassa yozuvlari', value: sum(d.cashRows, amt), count: (d.cashRows || []).length,
          note: 'sotuv, qarz to\'lovi, avans, oylik va qo\'lda kiritilganlar', to: '/gen_info?tab=cash&all=1' },
        { label: 'Kirim (naqd)', value: sum(d.incomeRows, amt), count: (d.incomeRows || []).length, to: '/gen_info?tab=cash&all=1' },
        { label: 'Chiqim (naqd)', value: -sum(d.expenseRows, amt), count: (d.expenseRows || []).length, to: '/gen_info?tab=cash&all=1' },
    ...
    return {
      ...
      parts: [
        ...
        { label: 'Bank yozuvlari', value: sum(d.bankRows, amt), count: (d.bankRows || []).length,
          note: 'sotuv, qarz to\'lovi, avans va boshqalar', to: '/gen_info?tab=bank&all=1' },
        { label: 'Kirim (bank)', value: sum(d.bankIncomeRows, amt), count: (d.bankIncomeRows || []).length, to: '/gen_info?tab=bank&all=1' },
        { label: 'Chiqim (bank)', value: -sum(d.bankExpenseRows, amt), count: (d.bankExpenseRows || []).length, to: '/gen_info?tab=bank&all=1' },
    ...
    return {
      ...
      parts: [
        { label: "Boshlang'ich qoldiq", value: Number(d.clickOpening?.amount || 0),
          ...
          opening: 'click' },
        { label: 'Click yozuvlari', value: sum(d.clickRows, amt), count: (d.clickRows || []).length, to: '/gen_info?tab=click&all=1' },
        { label: 'Kirim (click)', value: sum(d.clickIncomeRows, amt), count: (d.clickIncomeRows || []).length, to: '/gen_info?tab=click&all=1' },
        { label: 'Chiqim (click)', value: -sum(d.clickExpenseRows, amt), count: (d.clickExpenseRows || []).length, to: '/gen_info?tab=click&all=1' },
